...
if __name__ == '__main__':
        ...
        for filename in os.listdir(gemma_class_dir):
            ...
            if "en" in filename:
                with open(txt_path, 'r', encoding='utf-8') as file:
                    en_street_text = file.read()
                
                jpg_filename = filename.replace("en-", "")
                jpg_filename = jpg_filename.replace("txt", "jpg")
                jpg_path = os.path.join(gemma_class_dir, jpg_filename)

                en_street_list = en_street_text.split(";")
                for en_street_item in en_street_list:
                    trl_message = format_data_for_trl(jpg_path, en_street_item)
    ...
    image = train_dataset[2]["messages"][0]["content"][1]["image"]
    print("Посмотрим, что выдаст необученная модель")
    ...
    model.save_pretrained("lora_model_1")  # Local saving
    processor.save_pretrained("lora_model_1")