...
for class_n in tqdm(os.listdir(CLASSES_DIR)):
    ...
    for filename in os.listdir(class_dir):
        ...
        sorted_list = found_similar_images(current_file_path, images_db, 5)

        images_n += 1
        for similar_class in sorted_list:
            if (int(class_n) == int(similar_class["class_n"])) and (similar_class["filename"] != filename):
                accuracy_n += 1
                break

formatted_accuracy = accuracy_n/images_n * 100
