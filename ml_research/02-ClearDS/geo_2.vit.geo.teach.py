    ...
    def __init__(self, Transform):
        ...
        for sn_path in subfolders:
            ...
            for image_path in images_paths:
                item = {}
                item["src"] = image_path
                item["id"] = str(os.path.basename(image_path).split('.')[0])
                item["y"] = sn_id
                DSList.append(item)
...
if __name__ == '__main__':
    ...
    full_dataset = GetData(TrainTransform)
    train_set, valid_set = random_split(full_dataset, [0.8, 0.2])
    ...
    for e in range(EPOCHS):
        ...
        for data, labels, ids in tqdm(trainloader):
            ...
            if (prev_train_loss>0):
                for id in ids:
                    try:
                        train_items_losses[str(id)] += float(loss.item() / BATCH / prev_train_loss)
                    except:
                        train_items_losses[str(id)] = float(loss.item() / BATCH / prev_train_loss)
        ...
        for data, labels, ids in tqdm(validloader):
            ...
            if prev_valid_loss > 0:
                for id in ids:
                    try:
                        valid_items_losses[str(id)] += float(loss.item() / BATCH / prev_valid_loss)
                    except:
                        valid_items_losses[str(id)] = float(loss.item() / BATCH / prev_valid_loss)
